...
func main() {
        ...
        regexFlag := flag.String("regex", "", "Custom regex pattern to match (optional, skips base64 validation)")
        minCharsFlag := flag.Int("minchars", 4, "Minimum number of characters in encoded string to check (ignored if -regex is used)")
        ...
        // Create the regex dynamically based on the minimum characters or use the custom regex
        var regexPattern string
        customRegex := *regexFlag != ""
        if customRegex {
        ...
        if *fileFlag != "" {
                processFile(*fileFlag, re, customRegex, *debugFlag)
        } else if *dirFlag != "" {
                err := filepath.Walk(*dirFlag, func(path string, info os.FileInfo, err error) error {
                        ...
                        if !info.IsDir() {
                                processFile(path, re, customRegex, *debugFlag)
                        }
                        return nil
                })
                if err != nil {
...
func processFile(fileName string, re *regexp.Regexp, skipBase64Check bool, debugMode bool) {
        if debugMode {
        ...
        for {
                ...
                for _, match := range matches {
                        if skipBase64Check {
                                // If -regex flag is used, skip base64 validation and log the match directly
